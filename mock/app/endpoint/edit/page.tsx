...
export default function Edit(){
  ...
  async function getMockValue(){
    try{
      const res = await fetch(`http://${hostname}/api/endpoints/${mockId}`, {
      ...
      setEndpoint(apis.endpoint)
      setApiValue(JSON.stringify(apis.value))
      setApiSettings(JSON.stringify(apis.settings))
    }catch(e){
      setStatus(()=>{throw e})
    }
  }

  async function submitPostData(event: FormEvent<HTMLFormElement>){
    event.preventDefault()
    try{
      const body = {endpoint, apiValue, apiSettings}
      const res = await fetch(`/api/endpoints/${mockId}`, {
        method: 'PUT',
        headers: {'Content-Type': 'applicaton/json'},
        body: JSON.stringify(body),
      })
      if (res.status!=200){throw new Error("Internal server error")}
      router.push(`/endpoint/${mockId}`)
    }catch(e){
      setStatus(()=>{throw e})
    }
  }

  useEffect(()=>{getMockValue()},[])
